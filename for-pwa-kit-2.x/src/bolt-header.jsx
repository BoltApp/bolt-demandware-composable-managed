...
const BoltHeader = ({boltConfig}) => {
    ...
    return (
        <Helmet>
            {boltScriptTags && boltScriptTags.map((boltScriptTag) => (
                <script type="text/javascript" key={boltScriptTag.id} id={boltScriptTag.id} data-publishable-key={boltScriptTag.publishableKey} src={boltScriptTag.src}></script>
            ))}
        </Helmet>
    )
}
